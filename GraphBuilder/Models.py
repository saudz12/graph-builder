...
class Graph:
    ...
    def add_edge(self, u, v, weight, line_id):
        self.graph.setdefault(u, []).append((v, weight))

        self.edges.setdefault((u, v), []).append((weight, line_id))

        if not self.oriented:
            self.graph.setdefault(v, []).append((u, weight))

            self.edges.setdefault((v, u), []).append((weight, line_id))
        #print(self.graph)
...
class DrawEdge:
    def __init__(self, from_node_id, to_node_id, line_id, weight, flux_id):
